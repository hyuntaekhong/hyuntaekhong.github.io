...
이번에는 구구단을 출력하는 코드입니다. 해당 코드는 이중 for문으로 구성되어 있습니다. 바깥의 ```for(int i = 2; i <=9; i++) {...}```의 2부터 9까지 변하면서 총 8번 반복 실행됩니다. ```int i = 2;```일 때, ```int j```가 1~9까지 반복하며 2*j 계산값들을 출력합니다. 그 다음 ```int i = 3, 4, ... 9까지 해당 실행문이 반복 시행되며 구구단을 출력하게 됩니다.
...
break문은 반복문인 for문, while문, do-while문 실행을 중지할 때 사용합니다. braek문은 예제를 확인하며 알아보겠습니다.
